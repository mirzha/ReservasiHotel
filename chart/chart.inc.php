...
function writeShoppingchart() {
	...
	if (!$chart) {
		return '<h3><p>Detail Are not Available </h3></p>';
	} else {
		// Parse the chart session variable
		$items = explode(',', $chart);
		$s = (count($items) > 1) ? 's' : '';
		return '<h3> Your Reservation : <a href="index.php?mod=chart&pg=chart">' . count($items) . ' Room' . $s . ' Reservation </a></h3>';
	}
}
...
//	print_r($chart);
	if ($chart) {
		...
		$output[] = "<th><td>Room</td><td> Price</td><td>Room Booked</td><td>subtotal</td><td>Action</td></th>";
		...
		foreach ($contents as $id => $qty) {
			...
			$output[] = '<td><a href="index.php?mod=chart&pg=chart&action=delete&id=' . $id . '" class="btn btn-danger">Cancel Reservation</a></td></tr>';
			$no++;
		}
		$output[] = "</table>";
		$qty = getQty();

		$output[] = '<h3>Total Payment	: ' . format_rupiah($total) . '</h3>';

		$_SESSION['totalbayar'] = $total;
		$output[] = '<button type="submit" class=\'btn btn-primary\'> Update Reservation </button>';
		$output[] = '</form>';
		$output[] ='<a href=\'chart/chart_action.php\' class=\'btn btn-primary\'> Check out </a>';
	} else {
		$output[] = '<p>You Have not Made Any Reservation yet</p>';
	}
	return join('', $output);
}
