...
<br>
			<div class="block">
				<h4 class="title"><strong>Suggested</strong> Room</h4>
			<?php	
...
?>
					...
					?>	</a>
						...
						<p>
							<span class="label label-warning">Room Available : <?php echo   get_status_stok($rows->jumlah)?></span>
						</p>
						<?php
						if(!empty($_SESSION['idpelanggan']) && ($rows->jumlah>0)){ ?>
						<a href='index.php?mod=chart&pg=chart&action=add&id=<?=$rows->idproduk?>' class='btn btn-warning'><i class='icon-shopping-cart icon-white'></i>Reserve</a>
						<?php } ?>
					</div>
				</li>
		<?php } ?>
